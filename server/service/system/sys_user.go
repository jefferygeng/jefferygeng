...
func (userService *UserService) AppLogin(u *system.SysUser) (err error, userInter *system.SysUser) {
	var user system.SysUser
	u.Password = utils.MD5V([]byte(u.Password))
	err = global.GVA_DB.Where("mobile = ? AND password = ?", u.Username, u.Password).Preload("Authorities").Preload("Authority").First(&user).Error
	return err, &user
}
...
func (userService *UserService) SMSLogin(u *system.SysUser) (err error, userInter *system.SysUser) {
	var user system.SysUser
	err = global.GVA_DB.Where("mobile = ?", u.Mobile).Preload("Authorities").Preload("Authority").First(&user).Error
	return err, &user
}
...
func (userService *UserService) ForgetPassword(u *system.SysUser, newPassword string) (err error, userInter *system.SysUser) {
	var user system.SysUser
	err = global.GVA_DB.Where("mobile = ?", u.Mobile).First(&user).Update("password", utils.MD5V([]byte(newPassword))).Error
	return err, u
}
